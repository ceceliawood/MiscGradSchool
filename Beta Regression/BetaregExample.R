...
frame2 <- read_csv('https://raw.githubusercontent.com/ceceliawood/MiscGradSchool/refs/heads/main/Beta%20Regression/Data%20Mining_all.csv') %>% 
  mutate(Gt_pct = Gt_pct/100, # turn percents into decimals
         Lp_pct = Lp_pct/100,
         Fh_pct = Fh_pct/100,
         Mt_pct = Mt_pct/100) %>% 
  ...
  mutate(across(contains('RV'), ~if_else(. == 0, 0.0001, .))) %>% 
  mutate(across(contains('RV'), ~if_else(. == 1, 0.9999, .))) %>%
  mutate(EV5 = if_else(EV5 == 0, 0.001, EV5)) %>%
  mutate(EV6 = if_else(EV6 == 0, 0.001, EV6)) %>% 
  mutate(EV7 = if_else(EV7 == 0, 0.001, EV7)) %>% 
  mutate(EV8 = if_else(EV8 == 0, 0.001, EV8))
...
RV2_logit <- betareg(RV2 ~ EV1 + EV2 + EV3 + log(EV4) + log(EV5) + log(EV6) + log(EV7) + EV8 | EV5, data = frame1,
                    link = 'logit')
summary(RV2_logit)
plot(RV2_logit)
...
RV2_logit2 <- betareg(RV2 ~ EV1 + EV2 + EV3 + log(EV4) + log(EV5) + log(EV6) + log(EV7) + EV8 | log(EV5), data = frame1,
                     link = 'logit')
summary(RV2_logit2)
plot(RV2_logit2)
...
RV2_logit2b <- betareg(RV2 ~ EV1 + EV2 + log(EV5) + log(EV6) + EV8 | EV5, data = frame2,
                      link = 'logit')
summary(RV2_logit2b)
plot(RV2_logit2b)


RV2_logit2b2 <- betareg(RV2 ~ EV1 + EV2 + EV4 + log(EV5) + log(EV6)+ EV8 | EV5, data = frame2,
                       link = 'logit')
summary(RV2_logit2b2)

lrtest(RV2_logit2b, RV2_logit2b2)
...
test <- lm(frame2$RV4 ~ sqrt(frame2$EV4))
summary(test)
